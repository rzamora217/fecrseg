...
CHRadSinkFeSI::CHRadSinkFeSI(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
CHRadSinkFeSI::computeQpResidual()
{
  ...
  for (int i=0; i<_n; ++i)
  {
    for (int j=i+1; j<_n; ++j)
    {
      ...
      if(_vUIm_i[_qp][ind]>(1e-16))
      {
        Real inside_g_i = 100.0*ytot + exp(-_vUIb_i[_qp][ind]/_vUIm_i[_qp][ind]);
	    Real dgidc = 100.0*_vUIm_i[_qp][ind]*poly4Log(inside_g_i,tol,1);
	    Real g_i = _vUIm_i[_qp][ind]*poly4Log(inside_g_i,tol,0) + _vUIb_i[_qp][ind];
	    RealGradient grad_g_i = grad_ytot*dgidc;
	    outval1 += -grad_g_i*f_i + (_vUIC_i[_qp][ind] - g_i)*grad_f_i;
      }else{
	    outval1 += _vUIC_i[_qp][ind]*grad_f_i;
      }
	  ind++;
    }
  }
...
Real
CHRadSinkFeSI::computeQpJacobian()
{
  ...
  for (int i=0; i<_n; ++i)
  {
    for (int j=i+1; j<_n; ++j)
    {
      ...
      if(_vUIm_i[_qp][ind]>(1e-16))
      {
	    ...
	    RealGradient grad_g_i = grad_ytot*dgidc;
	    outval1 += -grad_g_i*f_i + (_vUIC_i[_qp][ind] - g_i)*grad_f_i;
	    doutval1+=(-_vUIm_i[_qp][ind]*100*_phi[_j][_qp]*poly4Log((100*ytot+exp(-_vUIb_i[_qp][ind]/_vUIm_i[_qp][ind])),tol,1)*grad_f_i-((_vUIm_i[_qp][ind]*100*_grad_phi[_j][_qp]*(100*ytot+exp(-_vUIb_i[_qp][ind]/_vUIm_i[_qp][ind]))-100*_phi[_j][_qp]*_vUIm_i[_qp][ind]*100*grad_ytot)*poly4Log(((100*ytot+exp(-_vUIb_i[_qp][ind]/_vUIm_i[_qp][ind]))),tol,2)*(-1))*f_i);
      }else{
	    outval1 += _vUIC_i[_qp][ind]*grad_f_i;
      }
	  ind++;
